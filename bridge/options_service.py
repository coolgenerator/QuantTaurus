...
CLOSE_DTE = int(env("QT_OPT_CLOSE_DTE", "7"))      # 距到期≤N天无条件平仓
TP_PCT = float(env("QT_OPT_TP", "1.00"))           # 权利金止盈
SL_PCT = float(env("QT_OPT_SL", "-0.50"))          # 权利金止损（基准）
# IV 自适应止损：合约 IV 高于此阈值时止损放宽 1.2 倍（高IV合约权利金波动大，
# 固定-50%容易被噪声打掉），低于一半阈值时收紧到 0.8 倍
IV_WIDE_THRESHOLD = float(env("QT_OPT_IV_WIDE", "60"))
