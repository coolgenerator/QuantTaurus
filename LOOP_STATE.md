...
## P22 自进化模式（用户loop指令"全宇宙跑模型+自进化+回测到正反馈"，2026-06-10 落地）

- 设计文档 docs/self-evolution-design.md：三层架构（引擎GA确定性 / 调度sweep /
...
- 正反馈状态：✅闸门通过率73%且拒绝合理 ✅跨家族竞争有效 ⏳前向验证（4周后首报）
  ⏳auto-sweep churn 观察（若频繁换冠军→升margin）
- 监督层每轮唤醒任务：① sweep结果审查 ② 组合净值 vs 回测预期 ③ churn率 ④ 新假设
- **Day-1 前向战绩（2026-06-11 收盘）**：组合均值 **+0.11%**（44会话：盈12/平24/亏8，
  截面σ1.5%）。Top: SOXX +4.35%/AMAT +2.98%（半导体链）；Bottom: NET -7%
  （满仓多遇跳空下杀，卖出共振离场已实现）/SOL空 -2.72%/GOOGL -1.76%。
  早盘最深 -0.39% 全部回血，组合分散有效。监督要点：隔夜跳空止损案例（NET型）
  累计频率；若高频→考虑开盘缓冲带风控
- 运维沉淀（44槽规模化连环修复，全部commit）：Yahoo节流闸门+尾部冷却+抖动错峰+
  双重退避 / 报价轮询15s→60s / paper摘要瘦身27倍+统计启动预热 / 期权桥配额分级+
  后台预热+锁超时快照 / plan单槽容错+置信度排序
...
- ②RuleVote 策略家族：买入规则子集投票进场+固定持有+卖出共振提前离场（长仓only），
  ...
  **RuleVote 统治终代基因池 24/24，晋升新 QQQ 冠军**——ensemble of 5×rule_vote，
  折 [1.48,1.43,0.88,1.27] 全正 / 留出 1.34（旧冠军 0.52）。
  常被选中规则：KDJ低位金叉、神奇九转·九买、启明星、多头排列成立、倒锤线
- ⚠️ 单 seed 单标的；QQQ 模拟盘已随新冠军重置（equity=1.0 重新起算前向验证）
- 下一步候选：其他5槽跑 RuleVote 实验 / F8 嵌套划分 / ③规则事件做挖掘因子
...
- [x] 批次5：SuperTrend(10,3) 主图双轨线 + 翻转信号（SPY 2年各9次）+ ADX(14) 芯片
      （当前 SPY ADX 27.6 强趋势）。Ichimoku/Keltner/吊灯 等暂缓（观感价值<成本）
- **P20 核心完成（2026-06-10 11:3x）**：经典信号层 36 种规则、六大类，SPY 2年222信号。
  剩余📋为低优先（TD13/OBV背离/缺口/CCI/箱体/52周新高低），按需再开
- 前向印证记录：06-04 MACD+KDJ死叉共振、06-08 MACD+RSI双顶背离、06-10 双顶颈线破位
  ——三个卖点信号都出现在本周下杀前/中，方向全对（仅轶事级样本，非统计证据）
