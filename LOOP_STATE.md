...
- [ ] F1 qmine crate：因子表达式 AST + 面板求值 + 横截面 IC/ICIR 适应度
      + 遗传规划挖掘循环（复杂度惩罚/时间折稳定性）← 本轮
- [ ] F2 数据面板：52股票池×10年日线对齐矩阵（server 集成，含留出切分）
- [ ] F3 挖掘运行 + 因子库持久化（factors.json：表达式/IC血统/正交性）
- [ ] F4 横截面多空策略构建（top-k 正交因子 z 分加权）+ 含成本回测 + 闸门
- [ ] F5 预测面板：当前因子值 → 下期相对走势排序预测 + IC 历史置信
- [ ] F6 前端「因子实验室」Tab + 策略家族档案视图（agent 并行做中）
- [ ] F7 agentic 回路：每轮 loop 读结果提新算子假设（写入 MINING_NOTES.md）
